...
public class SparqlClient {
  ...
  public <R> R executeSelectQuery(String query,
    ...
    while (true) {
      ...
      try (var qExec = QueryExecutionHTTP.newBuilder().httpHeaders(buildHttpHeaders())
          ...
          .query(query).build()) {
        retryCount += 1;
        return resultHandler.apply(qExec.execSelect());
      } catch (Throwable e) {
        if (mayRetry && retryCount != maxRetry) {
          log.warn("error '{}', will try in {} millis..", e, sleepMillis);
          sleepMillis *= 2;
  ...
  public Map<String, String> buildHttpHeaders() {
    Map<String, String> headers = new HashMap<>();
    getCurrentHttpRequest().map(r -> r.getHeader(HEADER_MU_SESSION_ID))
        .ifPresent(h -> headers.put(HEADER_MU_SESSION_ID, h));
    getCurrentHttpRequest().map(r -> r.getHeader(HEADER_MU_CALL_ID))
        .ifPresent(h -> headers.put(HEADER_MU_CALL_ID, h));
    if (isAuthSudo) {
      headers.put(HEADER_MU_AUTH_SUDO, "true");
    }
    return headers;
  }
